...
Perform the following checks on the non-UUID version 4.3.1:
...
1. Select **System upgrade** from the left panel to run the migration. Start on a folder with smaller data before running it on `/content/dam`. It is possible to migrate files in batches. However, it is important to migrate all files are migrated for system to be working. Only files which are not DITA assets and are not used in any DITA assets can be skipped for migration.

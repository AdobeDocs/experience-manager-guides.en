...
level: Experienced
---
# Configure a custom connector for the data sources
...
1. Define a standardized way for  the connectors to integrate with a system, allowing for the execution of queries, validation of connections, and and retrieving metadata.
...
1. Ensure the connectors are properly validated and can handle errors (`KonnectException`) when interacting with data sources.
...
| enabled| yes| <ul><li> This method checks if a `connector`, an entity or component connecting different parts of a system, is enabled. <li> By default, the method returns false, meaning that the connector is not enabled unless overridden by a class implementing this interface.| 
...
After setting up the connector interface,  you can implement the default connectors. There are five types of abstract connector implementations:
...
A connector uses a `Config` instance to execute and validate queries against an data source, forming a complete workflow.
...
For each abstract config class, you also get the concrete implementations you can use directly for authentication. These include:
...
Experience Manager Guides also allows you to provide custom resources for logos and templates along with the implementation. You can keep these resources in a resources folder.

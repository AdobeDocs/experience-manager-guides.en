...
|**Language**| Select the language you want the output to be translated. <br> **Note**: Cross-reference texts such as "see on chapter" or "see on page" are controlled by a language variable. This variable inherits the language defined for the parent topic unless a different language is explicitly set for child topics. If no language variable is specified, the system defaults to English (en-US) for rendering the output.| 

...
The new publishing engine for Native PDF, Native PDF engine v2, is based on an upgraded PDF generation framework and includes changes to font handling, CSS processing, and rendering behavior.
...
For example, the Native PDF engine v2 supports `OpenType` fonts, whereas the existing Native PDF engine v1 primarily relies on `TrueType` fonts. Similar rendering enhancements can affect the overall appearance of generated PDFs.

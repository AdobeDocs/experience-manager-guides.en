...
|Generate page names based on|  **Topic filename**: Uses the topic's file name to create the Site URL. <br> **Topic title**: Uses the topic's title to create the Experience Manager Site names. <br> **Note**: This option is available only when enabled for your environment. Contact Adobe Customer Success Team to enable it. When available, the value selected in this preset overrides the folder-level `aemsite.pagetitle` configuration. For more details, view [Customize existing AEM Site output](../install-conf-guide/customize-aem-site-output.md)|

...
The following is a list of answers to frequently asked questions that provides detailed insights into how Adobe Experience Manager Guides manages publishing workflows, scaling behavior, and infrastructure performance.It is intended for enterprise users, administrators, and documentation teams using AEM Guides for large-scale publishing.

## How many publishing requests can AEM Guides run per day?

The number of publishing requests AEM Guides can process per day depends on the size and type of content. By configuration, the system allows one publishing job per processor core. In the current setup, 20 publishing jobs can run in parallel (2 pods × 10 cores each).
...
## How does AEM Guides manage large files and graphics such as SVGs or .dita files exceeding 500 KB?
...
## What is the publishing infrastructure and configuration used by AEM Guides?

AEM Guides uses containerized, serverless microservices for publishing. Each new version of the publishing service is released as a Docker image, automatically deployed in Adobe's cloud environment. This design ensures:
...
No. AEM Guides is built with fault-tolerant architecture. If the publishing queue experiences overload, requests are automatically retried, and pods auto-scale to handle the additional load.Beyond a certain threshold, load throttling is applied to maintain stability. Other application areas (authoring, reviewing, asset management) remain unaffected.

## Is there a monitoring tool or log access available to identify when AEM Guides is under heavy load (similar to Jenkins monitoring)?

End customers currently do not have access to internal monitoring tools.For Adobe internal teams, monitoring can be done using:
...
When a user triggers a publishing request from the Presets tab in the Map Console, the following steps occur:
...
4. The serverless publishing service spins up a Docker container, downloads the assets, executes the publishing operation, and places the generated output artifacts back into AEM Guides.
This workflow ensures reliable, isolated, and scalable publishing execution.

## How much time does a map take before it starts publishing on the microservice container? What factors define this time?
...
Once on the serverless publishing service, total publishing time depends on:
...
## Can AEM Guides prioritize publishing requests in the queue (instead of First-Come, First-Serve)?

Currently, all publishing jobs are treated equally and follow a First-Come, First-Serve (FCFS) model.There is no prioritization mechanism available at this time.
However, in future releases, prioritization logic (e.g., based on job size or business importance) could be introduced as part of queue optimization enhancements.

## How does AEM Guides handle auto-scaling for publishing requests?

AEM Guides publishing infrastructure supports automatic scaling (autoscaling) based on load.When publishing demand increases:
...
If a publishing request fails due to a transient issue (e.g., network interruption, service timeout):
...
- Users can view failure status and retry publishing manually from the Map Console if needed.

## Can users monitor or track publishing job progress?

Yes.AEM Guides provides real-time job status updates in the Presets tab of the Map Console, including:

- Job start and completion time
- Current stage (zipping, dispatching, publishing, or uploading results)
- Error notifications (if any)
This helps users understand job progress and identify potential delays.

## What best practices can improve publishing performance in AEM Guides?

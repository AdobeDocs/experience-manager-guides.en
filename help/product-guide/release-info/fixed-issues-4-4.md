...
- In the Editor, the download map feature fails to trigger a pop-up notification when it is ready to download. (14626)
- In the Editor, the download map feature fails to download a map with baseline. (14622)
- Invalid DTD Error in the Experience Manager Guides. (14482)
- The title in the Editor tab gets truncated after a dot(.) character. (14372)
...
- The Editor throws application errors when adding a new reference or topic using a specialized schema in the **Layout** view. (14094)
...
- The **Favorites** collection in the Editor fails to load. (13495)
...
- Zip files are not recognized in the Editor, and you cannot drag and drop them. (12709)
...
- The rootmap setting persists in the Editor even if the user has not set it explicitly from the **User Preferences**. (11551)
- The content with some attributes applied is not being highlighted in **Author** or **Preview** mode. (11063)
- In the Editor, the **Key References** window opens when inserting the `varname` tag. (10940) 
...
- The Editor lacks the ability to wrap an element inside the possible parent elements. (8791)
- The Editor gets uninstalled after reinstallation of Adobe Experience Manager Guides version 4.3.1. (14519)
...
- **AEM Site** preset doesn't allow the selection of a template outside the `/content` hierarchy in the Editor. (14260)
...
- Missing **ditaval** option in folder profile level output presets created via Editor UI. (11903)
...
- **Baseline Filter** files are not working with File Name in the Editor. (13486)
- In the Editor, the baseline shows the title for the previous version instead of the selected version of the DITA file. (13444)
...
- Labels from the `labels.json` file appear in random order in the Editor. (10508)
...
- Symmetry issues occur in the side by side review panels of the previous and the current versions in the Editor. (14156)

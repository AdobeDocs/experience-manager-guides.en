...
- Generating AEM site output (using composite component mapping) for a map with the `map_title` variable in the output path fails and results in an error. (GUIDES-36968)
- When an output path with a trailing slash is specified in the Native PDF output preset, the UI automatically appends an additional trailing slash, resulting in a double‑slash path that causes the PDF upload to fail. (GUIDES-34932)

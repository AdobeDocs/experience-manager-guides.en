...
- In some cases, the **Topic list** tab in the Reports panel does not display any results, even when the map contains multiple topics. (GUIDES-56893) <br> **Workaround:** Re-index the affected content to rebuild the parent-map relationships. The topics are then displayed in the Topic list tab as expected.

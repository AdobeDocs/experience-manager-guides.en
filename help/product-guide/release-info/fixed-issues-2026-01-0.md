...
- When inserting a cross-reference to a topic, the icons for maps and topics appear identical and share the same CSS, making them difficult to distinguish.(GUIDES-36662)
...
- When trying to delete a label from a version for a topic or map in the **Version history** panel of Assets UI, the label is not getting removed. (GUIDES-38276)
- When saving a topic or map as a new version, you can't add multiple labels either directly or from the dropdown, and instead retains only a single combined label. (GUIDES-32716)
- When uploading assets with filename containing invalid characters, the asset fails to upload and displays an incorrect message **file are locked by other user** despite the asset being unlocked. (GUIDES-32680)

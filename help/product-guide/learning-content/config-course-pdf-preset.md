...
Once the preset is created, configure the PDF preset settings. The preset configuration options are organized under General, Metadata, Layout, Security, Print, and Advanced tabs.
...
|  **Embed multimedia files** | Select this option to embed supported multimedia assets, such as audio and video, directly in the generated PDF. When enabled, video content is represented using a poster image or thumbnail in the PDF and can be played in compatible PDF viewers that support embedded multimedia. For video playback, open the PDF in a supported application such as the latest version of Adobe Acrobat and trust the document when prompted. Most browser-based PDF viewers do not support embedded multimedia playback. |
